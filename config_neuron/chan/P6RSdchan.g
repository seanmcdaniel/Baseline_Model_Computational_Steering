...
// and use TABFILL to expand the table to 3000 entries.
        setfield NaF14  X_A->calc_mode 0   X_B->calc_mode 0
        call NaF14  TABFILL X 3000 0

        setfield NaF14  Y_A->calc_mode 0   Y_B->calc_mode 0
        call NaF14  TABFILL Y 3000 0
...
// Fill the X_A table with alpha values and the X_B table with (alpha+beta)
        ...
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield NaP14 X_B->table[{i}] {z}
            x = x + dx
        end

// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield NaP14  X_A->calc_mode 0   X_B->calc_mode 0
        call NaP14  TABFILL X 3000 0
...
// Fill the X_A table with alpha values and the X_B table with (alpha+beta)
        ...
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield AR14 X_B->table[{i}] {z}
            x = x + dx
        end

// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield AR14 X_A->calc_mode 0   X_B->calc_mode 0
        call AR14 TABFILL X 3000 0
...
// Fill the X_A table with alpha values and the X_B table with (alpha+beta)
        ...
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield KDR14 X_B->table[{i}] {z}
            x = x + dx
        end

// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield KDR14 X_A->calc_mode 0   X_B->calc_mode 0
        call KDR14 TABFILL X 3000 0
...
// Fill the Y_A table with alpha values and the Y_B table with (alpha+beta)
        ...
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield KA14 Y_B->table[{i}] {z}
            x = x + dx
        end

// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield KA14 X_A->calc_mode 0   X_B->calc_mode 0
        call KA14 TABFILL X 3000 0

        setfield KA14 Y_A->calc_mode 0   Y_B->calc_mode 0
        call KA14 TABFILL Y 3000 0
...
// Fill the Y_A table with alpha values and the Y_B table with (alpha+beta)
        ...
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield K214 Y_B->table[{i}] {z}
            x = x + dx
        end

// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield K214 X_A->calc_mode 0   X_B->calc_mode 0
        call K214 TABFILL X 3000 0

        setfield K214 Y_A->calc_mode 0   Y_B->calc_mode 0
        call K214 TABFILL Y 3000 0
...
// Fill the X_A table with alpha values and the X_B table with (alpha+beta)
        ...
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield KM14 X_B->table[{i}] {z}
            x = x + dx
        end

// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield KM14 X_A->calc_mode 0   X_B->calc_mode 0
        call KM14 TABFILL X 3000 0
...
// Fill the Y_A table with alpha values and the Y_B table with (alpha+beta)
        ...
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield CaL14 Y_B->table[{i}] {z}
            x = x + dx
        end

// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield CaL14 X_A->calc_mode 0   X_B->calc_mode 0
        call CaL14 TABFILL X 3000 0

        setfield CaL14 Y_A->calc_mode 0   Y_B->calc_mode 0
        call CaL14 TABFILL Y 3000 0
...
// Fill the X_A table with alpha values and the X_B table with (alpha+beta)
        ...
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield CaH14 X_B->table[{i}] {z}
            x = x + dx
        end

// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield CaH14 X_A->calc_mode 0   X_B->calc_mode 0
        call CaH14 TABFILL X 3000 0
...
// covering the range xmin = -0.1 to xmax = 0.05.
        ...
        call KCs14 TABCREATE X {xdivs} {xmin} {xmax}
        int i
...
// Expand the tables to 3000 entries to use without interpolation
    setfield KCs14 X_A->calc_mode 0 X_B->calc_mode 0
    setfield KCs14 Xpower 1
    call KCs14 TABFILL X 3000 0
...
//  rather than being used in a differential equation.
        ...
        call KCs14 TABCREATE Z {xdivs} {xmin} {xmax}
        int i
        float x,dx,y
        dx = (xmax - xmin)/xdivs
        x = xmin
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield KCs14 Z_A->table[{i}] {y}
            setfield KCs14 Z_B->table[{i}] 1.0
            x = x + dx
        end
        setfield KCs14 Z_A->calc_mode 0 Z_B->calc_mode 0
        setfield KCs14 Zpower 1
// Make it an instantaneous gate (no time constant)
    setfield KCs14 instant {INSTANTZ}
// Expand the table to 3000 entries to use without interpolation.
    call KCs14 TABFILL Z 3000 0
...
// covering the range xmin = -0.1 to xmax = 0.05.
        ...
        call KCd14 TABCREATE X {xdivs} {xmin} {xmax}
        int i
        float x,dx,alpha,beta
        dx = (xmax - xmin)/xdivs
        x = xmin
...
// Expand the tables to 3000 entries to use without interpolation
    setfield KCd14 X_A->calc_mode 0 X_B->calc_mode 0
    setfield KCd14 Xpower 1
    call KCd14 TABFILL X 3000 0
...
//  rather than being used in a differential equation.
        ...
        call KCd14 TABCREATE Z {xdivs} {xmin} {xmax}
        int i
        float x,dx,y
        dx = (xmax - xmin)/xdivs
        x = xmin
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            /* activation will be computed as Z_A/Z_B */
            setfield KCd14 Z_A->table[{i}] {y}
            setfield KCd14 Z_B->table[{i}] 1.0
            x = x + dx
        end
        setfield KCd14 Z_A->calc_mode 0 Z_B->calc_mode 0
        setfield KCd14 Zpower 1
// Make it an instantaneous gate (no time constant)
    setfield KCd14 instant {INSTANTZ}
// Expand the table to 3000 entries to use without interpolation.
    call KCd14 TABFILL Z 3000 0
...
// range from xmin = 0 to xmax = 1000, with 50 divisions
        ...
        call KAHPs14 TABCREATE Z {xdivs} {xmin} {xmax}
        int i
        float x,dx,y
        dx = (xmax - xmin)/xdivs
        x = xmin
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield KAHPs14 Z_A->table[{i}] {y}
            setfield KAHPs14 Z_B->table[{i}] {y + 10.0}
            x = x + dx
        end
// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield KAHPs14 Z_A->calc_mode 0   Z_B->calc_mode 0
        call KAHPs14 TABFILL Z 3000 0
...
// range from xmin = 0 to xmax = 1000, with 50 divisions
        ...
        call KAHPd14 TABCREATE Z {xdivs} {xmin} {xmax}
        int i
        float x,dx,y
        dx = (xmax - xmin)/xdivs
        x = xmin
        for (i = 0 ; i <= {xdivs} ; i = i + 1)
            ...
            setfield KAHPd14 Z_A->table[{i}] {y}
            setfield KAHPd14 Z_B->table[{i}] {y + 10.0}
            x = x + dx
        end
// For speed during execution, set the calculation mode to "no interpolation"
// and use TABFILL to expand the table to 3000 entries.
        setfield KAHPd14 Z_A->calc_mode 0   Z_B->calc_mode 0
        call KAHPd14 TABFILL Z 3000 0
// Use an added field to tell the cell reader to set up the
// CONCEN message from the Ca_concen element
