    ...
    for (i=1;i<=(P6RSd_NY);i=i+1)
        ...
        for (j=1;j<=(P6RSd_NX);j=j+1)
            ...
            if ( {randneur <= raninputexcitatory} )
                ...
                if ({output}==1)
                    echo P6RSd {mynode} {i} {j} is receiving random input (randneur=={randneur}<={raninputexcitatory}) at {Ranrate} Hz
                end
                ...
                setfield /P6RSdnet/P6RSd[{dex}]/apdend3/Ex_chSPIKEAMPA synapse[0].delay 0 synapse[0].weight {randominputweight}
