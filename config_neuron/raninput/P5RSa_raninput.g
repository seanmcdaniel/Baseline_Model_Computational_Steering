    ...
    for (i=1;i<=(P5RSa_NY);i=i+1)
        ...
        for (j=1;j<=(P5RSa_NX);j=j+1)
            ...
            if ( {randneur <= raninputexcitatory} )
                ...
                if ({output}==1)
                    echo P5RSa {mynode} {i} {j} is receiving random input (randneur=={randneur}<={raninputexcitatory}) at {Ranrate} Hz
                end
                ...
                setfield /P5RSanet/P5RSa[{dex}]/apdend3/Ex_chSPIKEAMPA synapse[0].delay 0 synapse[0].weight {randominputweight}
